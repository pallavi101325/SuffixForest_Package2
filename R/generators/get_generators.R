...
get_subset <- function(all_sets,items,n,data,index,i){
  if(index == n+1){
    #print("Hi")
  ...
  data[[index]] <- items[[i]]
  get_subset(all_sets,items,n,data,index+1,i+1)
  get_subset(all_sets,items,n,data,index,i+1)
}
...
get_all_subset <- function(items, n){
  ...
  get_subset(all_sets,items,n,data,1,1)
  return (all_sets)
}
    ...
    for(pat in keys(FCP)){
      if(length(pat[["itemset"]]) == i){
        ...
        while((generator_size < len) & found_gen == FALSE){
          all_subsets <- hashset()
          all_subsets <- get_all_subset(pat[["itemset"]], generator_size)
          for(subset in keys(all_subsets)){
            print("subset : ")
            print(subset)
            not_generator <- FALSE
            for(generator in keys(GEN)){
              if(identical(generator[["itemset"]],subset)){
                #print("setequal")
                not_generator <- TRUE
                break
              }
            ...
            if(not_generator == FALSE){
              ...
              for(pat2 in keys(cur_FCP)){
                if(is_subset(pat2[["itemset"]],subset)){
                  not_generator <- TRUE
                  break
                }
              }
